...
public:
    void insert(R row_key, C column_key, V value) {
        ...
        if (row_it == wrapped.end()) {
            ...
            wrapped.insert(std::make_pair(row_key, std::move(
                    row))); // std::move required here because unique_ptr would be copied
        } else {
...
namespace std {
    ...
    struct equal_to<std::weak_ptr<T>> {
        constexpr bool operator()(const std::weak_ptr<T>& lwp, const std::weak_ptr<T>& rwp) const {
            return &lwp == &rwp;
        }
    };
}
